...
public class ArrayMultiSet<E> implements Collection<E> {
  ...
  /**
   * Begins by checking if the backing store has an unassigned entry in which an additional element can be stored. If
   * (and only if) the backing store is full, it allocates twice as much space and copies all of the existing elements
   * over.
   */
  @SuppressWarnings("unchecked")
  private void checkAndExpandStore() {
	 if(size()==_store.length) {
		//if backing store full
		 E[] ans = (E[])(new Object[2*_store.length]);
		 //move it to bigger one
		 for(int i=0;i<_size;i++) 
			 ans[i]=_store[i];
			 _store=ans;
		 //copy tuff move around
	 }
  ...
  /**
   * Adds the element to the first available entry in the backing store and updates the instance variables to maintain
   * the class invariant.
   *
   * @param elem Element to be added to the backing store
   */
  private void addAtEnd(E elem) {
  _store[_size++]=elem;
  }
  ...
  public boolean contains(Object obj) {
    // Skipped on behalf of next week's homework.
