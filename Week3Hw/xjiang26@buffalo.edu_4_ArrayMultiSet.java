...
public class ArrayMultiSet<E> implements Collection<E> {
   ...
   * can assign null to the element that has been moved and then update any instance variables to insure class invariant
   * remain true.<br />
   ...
   *
   * @param indexFound Index of the element to remove from the MultiSet.
  ...
  private boolean removeAtIndex(int indexFound) {
	if(indexFound>=0 && indexFound<=_size){
		_store[indexFound]=null;
		_size=_store.length-1;
	}
	return true;
  }
  /**
