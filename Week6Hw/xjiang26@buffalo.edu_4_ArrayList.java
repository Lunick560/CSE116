...
/**
 * This defines the basic functionality of an array-based List. Students will complete several methods in this class to
 * show they understand how data can be added and removed from an array-based list.
 *
...
public class ArrayList<E> extends AbstractList<E> {
  ...
  /**
   * This method, which is only used within the ArrayList class, returns the instance to its initial state. This call
...
/**List<E> list = new ArrayList<E>();
  ...
  */
  ...
  /**
   * Removes the element found at the given index. This is done while maintaining the class invariant, by shifting
   * elements in the list to eliminate any "hole" that this removal could create. Once this is done, we can assign null
   * to the location that had been at the end of the ArrayList and update any instance variables to insure class
   * invariant remain true.<br />
   * <b>Precondition (e.g., this method SHOULD ASSUME this is true) </b>: {@code i} is a valid index within
   * {@code _store} AND {@code _size} is not 0.
   *
   * @param index Index of the element to remove from the ArrayList.
   * @return True is always returned since this method is called only when the removal is successful.
   */
...
return true;
   ...
   * element in the ArrayList, -1 is returned.
   *
   * @param obj Object (or null) for which we return the first index at which it is found in a valid location in _store
   * @return Index in _store at which the item is found or -1 if it is not in the ArrayList.
   */
  private int findFirstIndex(Object obj) {
    ...
    // Checked all VALID indices, so we now know nothing matches. In this case, we return -1
    return -1;
  }
   ...
   *
   * @return True when a new, larger, backing store is needed before an element can be added. False if we could add an
   *         element currently.
   */
   ...
   *
   * @return Array with all of the ArrayList's elements AND additional space in which elements could be added.
   */
  @SuppressWarnings("unchecked")
  private E[] growBackingStore() {
    // For maximal efficiency (and best practice), we double the size of the array each time it grows.
    E[] retVal = (E[]) new Object[_store.length * 2];
    // Copy all of the elements into the new array
    for (int i = 0; i < _store.length; i++ ) {
      retVal[i] = _store[i];
    }
    // An easier, more efficient way (but less useful for teaching) to allocate and copy the elements coding is:
    // _store = Arrays.copyOf(_store, _store.length * 2);
    return retVal;
  }

  /**
   * After shifting elements to make space in the array, this adds the element to the backing store and updates the
   * instance variables to maintain the class invariant. Returns if this addition was successful (this should always
   * return true).
   *
   * @param index Index at which the new element should be added.
