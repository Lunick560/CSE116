...
public class NthSmallest {
  ...
  /**
   * This method returns the nth smallest item in the array list WITHOUT having to sort the data. Doing this involves a
   * recursive process, however. Recursion is not required, but really strongly recommended.
   *
  ...
  /**
   * Recursively find the n-th smallest item. The method will do this by only considering the items whose index is
   * between the values of left and right, inclusive. Each recursion halves the number of elements under consideration.
   *
  ...
  private static int findItem(ArrayList<Integer> al, int left, int right, int sizeRanking) {
    ...
    pivotIdx = split(al, left, right, pivotIdx);
  if(sizeRanking == pivotIdx){
	  return al.get(pivotIdx);
  }
    if(sizeRanking < pivotIdx){
	  return findItem(al,left,pivotIdx-1,sizeRanking);
  }else{
    return findItem(al,pivotIdx+1,right,sizeRanking);
  }
  }

  /**
   * This is a helper method which splits the array list into left and right parts. The left part of the array list will
   * contain all of the values smaller than the pivot. The right part of the array list will contain all of the values
   ...
   * @param pivotIdx Index whose element will be our pivot -- all data will be partitioned based on whether it is larger
