...
public class ArrayBinaryTree<E extends Comparable<E>> extends AbstractSet<E> {
  ...
  private E leftChild(int idx) {
	//  if(store[(idx*2)+1]!=null){
	//	  return store[(idx*2)+1];
	//  }else if((idx*2)+1>=store.length){
	//	  return null;
	//  }
	//	  return null;
	  if(idx*2+1>=store.length)
	   {
	       return null;
	   }
	   else
  ...
  private E parent(int idx) {
	//  if((idx-1)/2>=store.length){
	//	  return null;
	//  }
	//  if(store[(idx-1)/2]!=null){
	//	  return store[(idx-1)/2];
	//  }else if(store[(idx-2)/2]!=null){
	...
	//  return null;
	  ...
	  if(idx>=store.length)
	   ...
	   {
	       if(store[(idx-1)/2]==null||idx==ROOT)
	       {
	           return null;
	       }
	       else
